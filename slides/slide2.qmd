...
## Modelling (unrestricted) counts with Poisson regression 
...
## Case study: attitude to genetically modified foods {background-color="#F5EDDE" visibility="hidden"}
```{r}
#| code-fold: true
...
```{r}
#| echo: false
xtabs(Counts ~ Income + Attitude, data = gmfood) 
```
...
- The data, collected between December 1996 and January 1997, shows the counts of the attitude of Australians to genetically modified (gm) foods according to their income.
...
## Contingency tables  {visibility="hidden"}




## Poisson regression for contingency tables  {visibility="hidden"}
